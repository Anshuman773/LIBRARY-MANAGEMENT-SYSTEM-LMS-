...
struct AuthorHashNode
{
    char key[14];
    long ISBN;
    struct AuthorHashNode *next;
};

struct AuthorHashTable
{
    struct AuthorHashNode *table[TABLE_SIZE];
};
void AuthorinitializeHashTable(struct AuthorHashTable *AuthorhashTable)
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        AuthorhashTable->table[i] = NULL;
    }
}
...
struct titleHashNode
{
    char key[14];
    long ISBN;
    struct titleHashNode *next;
};

struct titleHashTable
{
    struct titleHashNode *table[TABLE_SIZE];
};
void titleinitializeHashTable(struct titleHashTable *titlehashTable)
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        titlehashTable->table[i] = NULL;
    }
}

unsigned int titlehashFunction(const char *key)
{
    unsigned int hash = 7573;
    while (*key)
    {
...
unsigned int AuthorhashFunction(const char *key)
{
    unsigned int hash = 7573;
    while (*key)
    {
...
void AuthorinsertBook(struct AuthorHashTable *AuthorhashTable, const struct Book *newBook)
{
    unsigned int index = AuthorhashFunction(newBook->author);

    struct AuthorHashNode *newNode = (struct AuthorHashNode *)malloc(sizeof(struct AuthorHashNode));
    if (newNode == NULL)
    {
    ...
    strncpy(newNode->key, newBook->author, sizeof(newNode->key) - 1); // Copy author's name into newNode->key
    newNode->key[sizeof(newNode->key) - 1] = '\0';                    // Ensure null-termination
    newNode->ISBN = newBook->ISBN;
    newNode->next = NULL;

    if (AuthorhashTable->table[index] == NULL)
    {
        AuthorhashTable->table[index] = newNode;
    }
    else
    {
        newNode->next = AuthorhashTable->table[index];
        AuthorhashTable->table[index] = newNode;
    }
}

void AuthorsearchBooksHash(struct HashTable *hashTable, struct AuthorHashTable *AuthorhashTable, const char *searchType, const char *searchTerm)
{
    ...
    unsigned int index = AuthorhashFunction(searchTerm); // Get the index based on the search term
    struct AuthorHashNode *AuthorNode = AuthorhashTable->table[index];
    ...
    {
        if (strcmp(searchTerm, AuthorNode->key) == 0)
        { // Compare the search term with the author's name
            long ISBN = AuthorNode->ISBN;

            unsigned int index = hashFunction(ISBN); // Get the index based on the ISBN
            struct HashNode *current = hashTable->table[index];

            while (current != NULL)
            {
                struct Book *book = &(current->value);
                if ((strcmp(searchType, "ISBN") == 0 && book->ISBN == ISBN) ||
                    (strcmp(searchType, "Title") == 0 && strcmp(searchTerm, book->author) == 0))
                { // Compare search type and term with book's ISBN or Title
                    printf("%-4d", ++count);
...
void titleinsertBook(struct titleHashTable *titlehashTable, const struct Book *newBook)
{
    unsigned int index = titlehashFunction(newBook->title);

    struct titleHashNode *newNode = (struct titleHashNode *)malloc(sizeof(struct titleHashNode));
    if (newNode == NULL)
    {
        printf("Memory allocation failed.\n");
        return;
    ...
    strncpy(newNode->key, newBook->title, sizeof(newNode->key) - 1); // Copy title's name into newNode->key
    newNode->key[sizeof(newNode->key) - 1] = '\0';                   // Ensure null-termination
    newNode->ISBN = newBook->ISBN;
    newNode->next = NULL;

    if (titlehashTable->table[index] == NULL)
    {
        titlehashTable->table[index] = newNode;
    }
    else
    {
        newNode->next = titlehashTable->table[index];
        titlehashTable->table[index] = newNode;
    }
}

void titlesearchBooksHash(struct HashTable *hashTable, struct titleHashTable *titlehashTable, const char *searchType, const char *searchTerm)
{
    clock_t startTime = clock();
    ...
    printf("%-4s%-35s%-35s%-35s%-20s%-15s%-15s\n", "No.", "Title", "title", "ISBN", "Genre", "Rack", " Quantity");
    printf("=====================================================================================================================================================\n");

    unsigned int index = titlehashFunction(searchTerm); // Get the index based on the search term
    struct titleHashNode *titleNode = titlehashTable->table[index];
    ...
    {
        if (strcmp(searchTerm, titleNode->key) == 0)
        { // Compare the search term with the title's name
            long ISBN = titleNode->ISBN;
            ...
            while (current != NULL)
            {
                struct Book *book = &(current->value);
                if ((strcmp(searchType, "ISBN") == 0 && book->ISBN == ISBN) ||
                    (strcmp(searchType, "Title") == 0 && strcmp(searchTerm, book->title) == 0))
                { // Compare search type and term with book's ISBN or Title
                    printf("%-4d", ++count);
                    printBook(&(current->value));
                }
...
void automatesearchBooks(const char *searchType, const char *searchTerm)
{
    FILE *readfile;
    readfile = fopen("fake.csv", "r");
    ...
    // Append search details to a file
    FILE *log_file = fopen("LCS-automate-search_time.txt", "a");
    if (log_file != NULL)
    {
        fprintf(log_file, "Search Type: %s, Search Term: %s, Time Taken: %.4f seconds, Length of the word: %04d\n",
                searchType, searchTerm, time_taken, (int)strlen(searchTerm));
...
void automatetime()
{
    char arr[100000][14]; // Array to store ISBNs, each ISBN can be of max length 13 + 1 (for null terminator)
    int ct = 0;           // Counter for ISBNs
    ...
    while (fgets(line, sizeof(line), readfile))
    {
        ...
        if (title != NULL && author != NULL && isbn != NULL && genre != NULL && rack != NULL && quantity != NULL)
        {
            strncpy(arr[ct], isbn, 13); // Copy ISBN to the array
            arr[ct][13] = '\0';         // Null-terminate the string
            ct++;
        }
        else
        {
        ...
        // If we have reached the end of relevant data, break out of the loop
        if (ct >= 100000)
        {
            break;
        }
    ...
    for (int i = 0; i < ct; i++)
    {
        automatesearchBooks("ISBN", arr[i]);
    }
}
...
void automaticSearchHash(const struct HashTable *hashTable)
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        struct HashNode *current = hashTable->table[i];
        while (current != NULL)
        {
            automaticsearchBooksHash(hashTable, "ISBN", current->value.ISBN);
            current = current->next;
        }
    }
}

void automaticappendSearchTimeHashT(const char *searchType, const char *searchTerm, double timeTaken)
{
    FILE *timeFile = fopen("automatic_isbn_hashtimetaken.txt", "a");
    int length = (int)strlen(searchTerm);
    ...
    fprintf(timeFile, "Search Type: %s, Search Term: %s, Time Taken: %.4f seconds, Length of the word: %d\n", searchType, searchTerm, timeTaken, length);

    fclose(timeFile);
}
void automaticsearchBooksHash(struct HashTable *hashTable, const char *searchType, const char *searchTerm)
{
    clock_t startTime = clock();
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        ...
        while (current != NULL)
        {
            if ((strcmp(searchType, "ISBN") == 0 && lcs(searchTerm, current->key)) ||
                (strcmp(searchType, "Author") == 0 && lcs(searchTerm, current->value.author)) ||
                (strcmp(searchType, "Title") == 0 && lcs(searchTerm, current->value.title)))
            {
            }
            current = current->next;
        }
    ...
    double timeTaken = ((double)(clock() - startTime)) / CLOCKS_PER_SEC;
    automaticappendSearchTimeHashT(searchType, searchTerm, timeTaken);
}


void Author_automaticappendSearchTimeHashT(const char *searchType, const char *searchTerm, double timeTaken)
{
    FILE *timeFile = fopen("LCS-automate-author-hashtimetaken.txt", "a");
    int length = (int)strlen(searchTerm);
...
void Author_automaticsearchBooksHash(const struct HashTable *hashTable, const char *searchType, const char *searchTerm)
{
    clock_t startTime = clock();
    ...
    double timeTaken = ((double)(clock() - startTime)) / CLOCKS_PER_SEC;
    Author_automaticappendSearchTimeHashT(searchType, searchTerm, timeTaken);
}

void Author_automaticSearchHash(const struct HashTable *hashTable)
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        struct HashNode *current = hashTable->table[i];
        while (current != NULL)
        {
            Author_automaticsearchBooksHash(hashTable, "Author", current->value.author);
            current = current->next;
        }
    }
}
// Function declaration for TitleautomaticsearchBooksHash
void TitleautomaticsearchBooksHash(const struct HashTable *hashTable, const char *searchType, const char *searchTerm);

// Function declaration for TitleautomaticappendSearchTimeHashT
void TitleautomaticappendSearchTimeHashT(const char *searchType, const char *searchTerm, double timeTaken);

// Your existing functions
void TitleautomaticSearchHash(const struct HashTable *hashTable)
{
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        struct HashNode *current = hashTable->table[i];
        while (current != NULL)
        {
            TitleautomaticsearchBooksHash(hashTable, "Title", current->value.title);
            current = current->next;
        }
    }
}
...
void TitleautomaticsearchBooksHash(const struct HashTable *hashTable, const char *searchType, const char *searchTerm)
{
    ...
    double timeTaken = ((double)(clock() - startTime)) / CLOCKS_PER_SEC;
    TitleautomaticappendSearchTimeHashT(searchType, searchTerm, timeTaken);
}

void TitleautomaticappendSearchTimeHashT(const char *searchType, const char *searchTerm, double timeTaken)
{
    FILE *timeFile = fopen("LCS-automate-Title-hashtimetaken.txt", "a");
    int length = (int)strlen(searchTerm);
...
{
    const char *file_path = "D:\\DAA\\casestudy\\fake.csv"; // Replace with your actual file path

    int choice;
    do
    {
        printf("\nPress 1. To find book\nPress 2. To add book to inventory\nPress 3. To borrow book\nPress 4. To return book\nPress 5. To view books\nPress 6 to use Hash table\nPress 7. Automatic Searching\nPress 8. Automatic Searching using HashTable ...\nPress 9. to EXIT\n");
        scanf("%d", &choice);
        ...
        case 7:
            printf("Automatic Searching ..\n");
            automatetime();
            break;
        case 8:
            printf("Automatic Searching using HashTable ...\n");
            {
                printf("\nAuthomatic Searching with Hash Table");
                printf("\nPress 1. Search by ISBN\nPress 2. Search by Author\nPress 3. Search by Title\nPress 0. Go back\n");
                int searchOption;
                scanf("%d", &searchOption);
                ...
                {
                case 1:
                    automaticSearchHash(&library);
                    break;
                case 2:
                    Author_automaticSearchHash(&library);
                    break;
                case 3:
                    TitleautomaticSearchHash(&library);
                    break;
                case 0:
                    break;
                default:
...
int main()
{
    ...
    loadBooksFromFile(&library, "D:\\DAA\\casestudy\\fake.csv");
